import os
print sys.argv
name = sys.argv[sys.argv.index(sys.argv[2])+4]
cellsize = sys.argv[sys.argv.index(sys.argv[2])+1]
imsize = sys.argv[sys.argv.index(sys.argv[2])+2]
phase_center = str(sys.argv[sys.argv.index(sys.argv[2])+3])
ms1 = sys.argv[sys.argv.index(sys.argv[2])+5]
ms2 = sys.argv[sys.argv.index(sys.argv[2])+6:]

vis = [ms1] + ms2
ms2_name = '_'.join([i.split('.ms')[0] for i in ms2])
print name, cellsize, imsize, phase_center, ms1, ms2

tclean(vis=vis,selectdata=True,field="",spw="",timerange="",uvrange="",\
       antenna="",scan="",observation="",intent="",datacolumn="data",imagename='%s_%s_%s_1_psf_CASA_%s' % (ms1.split('.ms')[0],name,ms2_name,cellsize),\
